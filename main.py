...
class Myroot(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # Luồng nhận tín hiệu Bluetooth
        Thread(target=self.recvsignal_thread, daemon=True).start()

    def update_time(self, dt):
        """Hàm này được gọi mỗi 5ms để cập nhật thời gian."""
        if self.running:
    ...
    def Start_timer(self, instance):
        ...
        if not self.running:
            self.running = True
            self.start_time = time.time() - self.elapsed_time  # Để tiếp tục từ thời gian trước
            sendsignal("1")
            Clock.schedule_interval(self.update_time, 0.005)  # Cập nhật nhanh hơn
    ...
    def Stop_timer(self, instance):
        ...
        if self.running:
            self.running = False
            sendsignal("0")
            Clock.unschedule(self.update_time)  # Ngừng cập nhật thời gian
    ...
    def recvsignal_thread(self):
        """Luồng riêng để nhận tín hiệu Bluetooth"""
        while True:
            signal = recvsignal()
            if signal == "0":
...
def recvsignal():
    ...
    try:
        ...
        data = recv_stream.read(1024)  # Chỉ đọc 1024 bytes, không cần buffer riêng
